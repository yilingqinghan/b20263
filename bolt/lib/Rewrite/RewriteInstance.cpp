...
bool RewriteInstance::analyzeRelocation(
  ...
  if (BC->isRISCV32())
    Addend = getRelocationAddend(InputFile, Rel, false);
  else
    Addend = getRelocationAddend(InputFile, Rel, true);
...
void RewriteInstance::emitAndLink() {
  ...
  if (!DataSection)
    // Some ELF inputs place writable globals solely in the small-data section.
    DataSection = BC->getUniqueSectionByName(".sdata");
  Linker = std::make_unique<JITLinkLinker>(*BC, std::move(EFMM));
  // Use the layout delta computed from the transformed code.
  if (!opts::MinimizeRodata)
    Linker->loadObject(ObjectMemBuffer->getMemBufferRef(),
                       [this](auto MapSection) { mapFileSections(MapSection); },
                       DataSection->getAddress(), &BC->sizeChange);
  else
    Linker->loadObject(ObjectMemBuffer->getMemBufferRef(),
                       [this](auto MapSection) { mapFileSections(MapSection); },
  ...
  if (BC->HasRelocations) {
    ...
    if (opts::UseOldText) {
      ...
      if (CodeSize <= BC->OldTextSectionSize) {
        // JITLink applies data-reference fixups after this callback. Record the
        // exact reclaimable tail of the original text section, including all
        // section alignment selected by the final layout.
        BC->sizeChange = static_cast<int64_t>(
            (BC->OldTextSectionSize - CodeSize) & ~uint64_t{1});
...
//    }
  ...
  for (const ELFShdrTy &Section : Sections) {
    ...
    if (!opts::MaxCompress || BinSec->getName() == ".shstrtab") {
