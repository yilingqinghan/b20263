...
class ELFJITLinker_riscv : public JITLinker<ELFJITLinker_riscv> {
  ...
  int64_t DataSectionBegin = std::numeric_limits<int64_t>::max();
  const int64_t *FixupOffset = nullptr;
  int64_t ExtraFixupOffset = 0;
...
public:
  ...
  ELFJITLinker_riscv(std::unique_ptr<JITLinkContext> Ctx,
                     ...
                     const int64_t *fixupOffset,
                     int64_t extraFixupOffset)
      : JITLinker(std::move(Ctx), std::move(G), std::move(PassConfig)),
        DataSectionBegin(dataSectionBegin),
        FixupOffset(fixupOffset), ExtraFixupOffset(extraFixupOffset) {}
private:
  int64_t getFixupOffset() const {
    return (FixupOffset ? *FixupOffset : 0) + ExtraFixupOffset;
  }

  int64_t getAdjustedTargetValue(const Edge &E) const {
    int64_t Value = (E.getTarget().getAddress() + E.getAddend()).getValue();
    if (Value >= DataSectionBegin)
      Value -= getFixupOffset();
    return Value;
  }
  ...
  Error applyFixup(LinkGraph &G, Block &B, const Edge &E) const {
    ...
    case R_RISCV_PCREL_HI20: {
      int64_t Value = getAdjustedTargetValue(E) - FixupAddress.getValue();
      int64_t Hi = Value + 0x800;
      if (LLVM_UNLIKELY(!isInRangeForImm(Hi, 32)))
        return makeTargetOutOfRangeError(G, B, E);
      uint32_t RawInstr = *(little32_t *)FixupPtr;
    ...
    case R_RISCV_PCREL_LO12_I: {
      ...
      int64_t Value = getAdjustedTargetValue(*RelHI20) -
...
void link_ELF_riscv(std::unique_ptr<LinkGraph> G,
  ...
  ELFJITLinker_riscv::link(std::move(Ctx), std::move(G), std::move(Config),
                           dataSectionBegin, fixupOffset, extraFixupOffset);
}
