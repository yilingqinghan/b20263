...
void BinaryFunctionPassManager::runAllPasses(BinaryContext &BC) {
  ...
  if (BC.isRISCV() || BC.isRISCV32()) {
    ...
    Manager.registerPass(std::make_unique<RISCVGPRelax>(NeverPrint));
    Manager.registerPass(std::make_unique<RISCVConstLUIReuse>(NeverPrint));
    Manager.registerPass(std::make_unique<RISCVReuseLUIAggressive>(NeverPrint));
    Manager.registerPass(std::make_unique<RISCVMemorySSA>(NeverPrint));
    ...
    Manager.registerPass(std::make_unique<RISCVStackMemForward>(NeverPrint));
    Manager.registerPass(std::make_unique<RISCVTailMerge>(NeverPrint));
    // FIXME 危险操作
    Manager.registerPass(std::make_unique<RISCVCompressPeephole>(NeverPrint));
